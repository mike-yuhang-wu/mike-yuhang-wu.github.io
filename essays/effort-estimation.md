...
In theory, a good effort estimation requires prior experience or at least a completed similar tasks as reference. However, we have neither: many place in this project require us to build things using Next.js that we never build before, not even in a homework assignments, and we did not have enough historical data or time to perform careful analysis on the time an issue will take.
...
One example was the issue of adding an email feature to Wonkes. I estimated it would take around ten hours, largely because I feared it would be complex. In reality, the task took about three hours of coding and one additional hour to configure an email service provider. ChatGPT played a major role in reducing this time by helping me quickly find providers with free or low-cost tiers. Despite the issue was successful solved quickly, the original effort estimate was inaccurate.

Another example was the implemention the homepage. We thought it would take about an hour total since it involved no user interaction, just displaying information using basic React components like `Container`, `Card`, and standard HTML elements like `h1` and `p`. It turns out that this issue took nearly three hours.
...
Each teammate tracked time differently. Some take their best guesses, and some use stopwatches. I found pure guessing unreliable because my luck is bad, and using a stopwatch disruptive because it broke my focus every time I had to start or stop it. Instead, I adopted a simple but reasonably accurate method, which is read the clock and estimate the time elapsed.
...
Tracking non-coding effort like designing user interfaces, planning component structures, searching documentation, or asking ChatGPT was more difficult. These activities often occurred frequently with short duration and were hard to isolate from coding-effort. For those, I relied on personal feeling and approximate guesses, prioritizing speed and simplicity over precision.
